...
export class EmployeeComponent implements OnInit {
  ...
  updateEmpSelect(value, event) {
    if (event.target.checked) {
      this.applicationService.empList.push(value);
    }
    else if (!event.target.checked) {
      let indexx = this.applicationService.empList.indexOf(value);
      this.applicationService.empList.splice(indexx, 1);
    }
    console.log(this.applicationService.empList);
  }

  public remove(item) {
    let index = this.userService.getEmpCollection().indexOf(item);
    if (index > -1) {
      this.userService.getEmpCollection().splice(index, 1);
  ...
  updateSkills() {
    ...
    this.empData.empList = this.applicationService.empList;
